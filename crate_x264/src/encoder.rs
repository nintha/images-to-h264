...
impl Encoder {
    ...
    pub unsafe fn from_raw(raw: *mut x264_t) -> Self {
        let mut params = MaybeUninit::uninit().assume_init();
        ...
        -> Result<(Data, Picture)>
    {
        ...
        let mut picture = MaybeUninit::uninit().assume_init();
        x264_picture_init(&mut picture);
        picture.i_pts = pts;
        picture.img = image;

        let mut len = 0;
        let mut stuff = MaybeUninit::uninit().assume_init();
    ...
    pub fn headers(&mut self) -> Result<Data> {
        let mut len = 0;
        let mut stuff = unsafe { MaybeUninit::uninit().assume_init() };
